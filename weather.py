...
# Rain and wind reduce the stronger team's Elo advantage slightly.
...
def fetch_weather_for_venue(venue_name, game_date=None):
    """
    Returns weather dict for a venue on a given date.
    ...
    if is_indoor:
        return {
            "temperature_c": 20, "rain_mm": 0, "wind_kmh": 0,
            "is_indoor": True,
            "condition": "Indoor venue — no weather effect",
            "impact": "No adjustment", "elo_adjustment": 0,
        }
    ...
    try:
        ...
        return {
            "temperature_c": round(temp, 1), "rain_mm": round(rain, 1),
            "wind_kmh": round(wind, 1), "is_indoor": False,
            "condition": condition, "impact": impact,
            "elo_adjustment": suppression,
        }

    except Exception:
        return {
            "temperature_c": 18, "rain_mm": 0, "wind_kmh": 10,
            "is_indoor": False, "condition": "Weather unavailable",
            "impact": "No adjustment", "elo_adjustment": 0,
        }
...
def weather_elo_adjustment(wx, home_elo, away_elo):
    """
    Returns (home_adj, away_adj).
    Bad weather benefits the underdog by reducing the stronger team's edge.
    """
